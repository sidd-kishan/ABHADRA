...
The script generates a `.hex` file with one instruction per line, where each instruction follows the format:


- `c` (bits 28–31): 4-bit opcode representing the Brainfuck command
- `a` (bits 0–27): 28-bit absolute instruction memory address

### Opcode Mapping

| Brainfuck | Opcode | Description                  |
|-----------|--------|------------------------------|
| `]`       | 0x0    | Jump if x is zero            |
| `[`       | 0x1    | Loop start, push address     |
| `-`       | 0x2    | Decrement data               |
| `+`       | 0x3/0x2/0x3 | Flip-sub-flip (simulate increment) |
| `<`       | 0x4    | Move data pointer left       |
| `>`       | 0x5/0x4/0x5 | Flip-sub-flip (simulate right move) |
| `.`       | 0xA    | Output data                  |
| `,`       | 0xB    | Input data                   |
...
- ✅ **Grouped multi-step commands** (`+`, `>`) mapped to multiple PIO instructions but share a **single instruction address**
- ✅ **Dynamic bracket linking** for `[` and `]` (including **nested loops**)
- ✅ Supports **Harvard architecture** — separates instruction memory from tape/data
- ✅ Outputs a `.hex` file ready to feed into DMA or TX FIFO of RP2040 PIO

---

## 🧮 Example Output

Given the Brainfuck code:
```brainfuck
++>+
